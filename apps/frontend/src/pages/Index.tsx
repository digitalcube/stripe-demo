...
const Price: FC<{
  ...
  return (
    <Card img={product.images ? product.images[0] : null}>
      ...
      <Text sx={{ fontSize: 'small' }}>
        ...
        {price.unit_amount.toLocaleString()} (per{' '}
...
const CustomerPortal: FC = () => {
  ...
  return (
    <>
      <PageHeader
        title="Stripe Customer Portal デモ"
        subtitle="シンプルな支払い管理ポータル"
      />
      <Container
        size="2"
        sx={{
      ...
      >
        ...
        >
          <p>
            StripeのCustomer
            IDとユーザー情報を紐付けすることで、その顧客向けのポータルページを用意することができます。
            <br />
  ...
  return (
    <>
      <Benefits
        ...
        img={
          <img
            src="./assets/Stripe-Hosted-Invoices.jpg"
        ...
        nodes={[
          {
            ...
            excerpt:
              'Stripe Billingを使うことで、簡単かつ拡張しやすい形であなたのサービスに定期課金を組み込むことができます。',
          },
          {
            title: 'Checkoutでシンプル/セキュアな購入フロー',
            excerpt:
              'Stripe CheckoutはStripeがホストする購入ページを利用できる仕組みです。自前で購入処理を実装する必要がなく、より手軽より安全に購入画面を導入できます。',
          },
          {
            title: '支払い管理はCustomer Portalで',
            excerpt:
              'Customer Portalを使うことで、カード情報の更新や支払い履歴の確認・プラン変更などのユーザー操作画面を簡単に提供できます。',
          },
        ]}
      />
      <PageHeader
        title="テスト商品一覧"
        subtitle="体験用のデモ商品です。クレジットカードはテスト用の番号「4242 4242 4242 4242」、年月やCVCはお好きな番号を入力してお試しください。"
      />
